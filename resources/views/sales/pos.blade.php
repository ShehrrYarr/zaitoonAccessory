        ...
        <div class="pos-main">
              ...
              <div style="margin-top:6px;">
                <select id="manual_batch_select"
                  ...
                  <option value="{{ $batch->barcode }}">
                    ...
                    $batch->accessory->description }}
                  </option>
                  @endforeach
                </select>
                <button type="button" class="btn-scan" onclick="addSelectedBatch()">Add</button>

                <div id="price-mode-badge" class="price-mode-badge" style="display:none;">
                  Pricing Mode: <strong id="price-mode-text">Selling Price</strong>
                </div>

                <div class="toggle-box">
                  <label style="display:flex; gap:10px; align-items:center; margin:0;">
                    <input type="checkbox" id="use_cost_price" disabled>
                    <span style="font-weight:600;">Sell to Vendor at COST price</span>
                    <span style="color:#666; font-size:12px;">(default: selling price)</span>
                  </label>
                </div>
              </div>
            </div>

            {{-- ✅ Preload batches data with SAFE FALLBACKS for cost/selling attribute names --}}
            <script>
              window.batchData = {};
                          @foreach($batches as $batch)
                            @php
                              // Try many possible column names for COST
                              $cp = $batch->getAttribute('cost_price')
                              ...
                              // Try many possible column names for SELLING
                              $sp = $batch->getAttribute('selling_price')
                                  ?? $batch->getAttribute('sellingPrice')
                                  ?? $batch->getAttribute('sale_price')
                                  ?? $batch->getAttribute('salePrice')
                                  ?? $batch->getAttribute('price')
                                  ?? ($batch->accessory->getAttribute('selling_price') ?? 0);
                            @endphp
          ...
          <div class="pos-cart">
            ...
            <div class="input-row">
              ...
              <input type="number" id="cart_discount" min="0" step="0.01" placeholder="Enter Discount Amount"
                oninput="handleDiscountMode('amount')">
            </div>
            ...
            <div class="col-md-4">
              <div class="card shadow-sm">
                <div class="card-body">
                  ...
                  <small class="text-muted">{{ $todaysRefunds['returns'] }} return(s), {{ $todaysRefunds['lines'] }}
                    item line(s)</small>
                </div>
              </div>
            </div>
...
<script>
  $(document).ready(function () {
    ...
    $('#vendor_id').on('change', function () {
      ...
      const costToggle = document.getElementById('use_cost_price');
    ...
    document.getElementById('use_cost_price').addEventListener('change', function() {
      applyPricingMode();
    });
  ...
  function isVendorSelected() {
    const v = document.getElementById('vendor_id');
    return !!(v && v.value);
  }

  function isCostModeEnabled() {
    const el = document.getElementById('use_cost_price');
    return isVendorSelected() && el && el.checked;
  }

  // returns number OR null if cost-mode but cost missing/0
  function unitPriceForBarcode(code) {
    const batch = window.batchData[code];
    if (!batch) return 0;

    if (isCostModeEnabled()) {
      const c = Number(batch.cost_price);
      if (!c || c <= 0) return null;
      return c;
    }

    return Number(batch.selling_price || 0);
  }

  function updatePriceModeBadge() {
    const badge = document.getElementById('price-mode-badge');
    const txt   = document.getElementById('price-mode-text');
    if (!badge || !txt) return;

    badge.style.display = '';

    if (!isVendorSelected()) {
      badge.classList.remove('cost');
      txt.textContent = 'Selling Price (Walk-in)';
      return;
    }

    if (isCostModeEnabled()) {
      badge.classList.add('cost');
      txt.textContent = 'COST Price (Vendor)';
    } else {
      badge.classList.remove('cost');
      txt.textContent = 'Selling Price (Vendor)';
    }
  }
  ...
  function applyPricingMode() {
    ...
    if (isCostModeEnabled()) {
      for (const it of cart) {
        const p = unitPriceForBarcode(it.barcode);
        if (p === null) {
          alert('Cost price is not available in POS data for batch ' + it.barcode + '.');
          const t = document.getElementById('use_cost_price');
    ...
    cart = cart.map(it => {
      const forced = unitPriceForBarcode(it.barcode);
      return { ...it, price: forced === null ? it.price : forced };
    });
  ...
  function scanBarcode() {
    ...
    if (existing && (Number(existing.qty) + newQty) > Number(batch.qty_remaining)) {
      return alert('Cannot add. Total qty would exceed remaining stock for batch ' + code);
    }

    const price = unitPriceForBarcode(batch.barcode);
    if (price === null) return alert('Cost price is not available in POS data for batch ' + code + '.');

    if (existing) {
      existing.qty = Number(existing.qty) + newQty;
      existing.price = price;
    } else {
      cart.push({ barcode: batch.barcode, accessory: batch.accessory, qty: newQty, price: price });
    }
  ...
  function renderCart() {
    ...
    cart.forEach((item, i) => {
      ...
      const priceReadonly = isCostModeEnabled() ? 'readonly' : '';

      tbody.innerHTML += `<tr>
        <td>${item.barcode}</td>
        <td>${item.accessory}</td>
        <td><input type="number" value="${item.qty}" min="1" style="width:50px;" onchange="updateQuantity(${i}, this.value)"></td>
        <td><input type="number" value="${Number(item.price).toFixed(2)}" min="0" step="0.01" style="width:70px;" ${priceReadonly} onchange="updatePrice(${i}, this.value)"></td>
  ...
  function updateQuantity(i, newQty) {
    ...
    const code = cart[i].barcode;
    const batch = window.batchData[code];
    if (batch && q > Number(batch.qty_remaining)) {
      alert('Cannot set quantity above remaining stock for batch ' + code);
      return renderCart();
    }
  ...
  function updatePrice(i, newPrice) {
    if (isCostModeEnabled()) {
      const forced = unitPriceForBarcode(cart[i].barcode);
      if (forced !== null) cart[i].price = forced;
      return renderCart();
    }
  ...
  function checkoutSale() {
    ...
    const use_cost_price = (document.getElementById('use_cost_price') && document.getElementById('use_cost_price').checked) ? 1 : 0;
